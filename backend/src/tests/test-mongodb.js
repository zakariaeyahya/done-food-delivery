...
try {
  ...
  if (uriMatch) {
    ...
    console.log('\n⚠️  Format d\'URI non reconnu');
  }
} catch (err) {
  console.log('\n⚠️  Impossible d\'analyser l\'URI:', err.message);
}
...
mongoose.connect(process.env.MONGODB_URI, connectionOptions)
  ...
  .catch((err) => {
    ...
    if (err.message.includes('authentication failed') || err.message.includes('bad auth')) {
      ...
      console.log('      ⚠️  Moins sécurisé mais pratique pour le développement');
